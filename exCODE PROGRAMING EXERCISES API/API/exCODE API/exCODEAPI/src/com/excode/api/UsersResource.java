...
public class UsersResource {
	...
	public Response deleteUser(@PathParam("userID") String userID, @FormParam("password") String password,
		...
		try {
			...
			String user_ID = (String) Jwts.parser().setSigningKey(userManager.getKey()).parseClaimsJws(token).getBody()
					.get("username");
			userManager.deleteUser(userID, password, user_ID);
	...
	public Response insertUser(@FormParam("userID") String userID, @FormParam("email") String email,
			@FormParam("password") String password, @FormParam("name") String nameUser, @Context UriInfo uriInfo) {
	...
	@PUT
	@Consumes("application/x-www-form-urlencoded")
	public Response changePassword(@PathParam("userID") String userID, @FormParam("password") String password,
			@FormParam("passwordNew") String passwordNew, @FormParam("token") String token) {
		...
		try {
			...
			userManager.changePassword(userID, password, passwordNew, userIDAuthour);
			return Response.serverError().status(200).type("text/plain").entity("Change Successfully").build();
		} catch (SignatureException e) {
			// don't trust the JWT!
			return Response.serverError().status(401).type("text/plain").entity("You do not have permissions!").build();
	...
	public Response auth(@FormParam("username") String userID, @FormParam("password") String password) {
		...
		if (userManager.checkCredentials(userID, password)) {
			...
			// Invalid user
			return Response.serverError().status(401).type("text/plain").entity("Wrong Credentials").build();
		}
